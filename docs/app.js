 ...
 * - Simple weekend plan + Google/Apple Maps directions link from favorites (needs coordinates)
 *
 * Berlin UX tweak:
 * - Restaurants/cocktailbars are hidden from the card list (often no photo),
 *   but are available via dropdowns in the planner.
 */
...
function favKey(ds) {
  return `tripkit_favorites_${ds}_v1`;
}
function loadFavs(ds) {
  try {
    return new Set(JSON.parse(localStorage.getItem(favKey(ds)) || "[]"));
  } catch {
    return new Set();
  }
}
function saveFavs(ds, favsSet) {
  localStorage.setItem(favKey(ds), JSON.stringify(Array.from(favsSet)));
}
let favs = loadFavs(currentDataset);

/* ---------------- Planner settings (per dataset) ---------------- */
function planSettingsKey(ds) {
  return `tripkit_plan_${ds}_v1`;
}

function mealKey(ds) {
  return `tripkit_meal_choices_${ds}_v1`;
}
...
function saveMealChoices(ds, obj) {
  try {
    localStorage.setItem(mealKey(ds), JSON.stringify(obj || {}));
  } catch {}
}
...
function defaultPlanSettings(ds) {
  ...
  return { days: 3, arrival: "afternoon", departure: "evening", mapProvider: "both", startTime: "10:00" };
}
function loadPlanSettings(ds) {
  const def = defaultPlanSettings(ds);
  try {
    const raw = JSON.parse(localStorage.getItem(planSettingsKey(ds)) || "null");
    if (!raw) return def;
    return { ...def, ...raw };
  } catch {
    return def;
...
function savePlanSettings(ds, settings) {
  localStorage.setItem(planSettingsKey(ds), JSON.stringify(settings));
}
...
async function getBestWikipediaInfo(poi) {
  ...
  let best = null;
  try {
    best = await fetchMwSummary(lang, title);
  } catch {}
...
function coordKey(ds) {
  return `tripkit_coord_overrides_${ds}_v1`;
}
function loadCoordOverrides(ds) {
  try {
    return JSON.parse(localStorage.getItem(coordKey(ds)) || "{}") || {};
  } catch {
    return {};
  }
}
...
function buildOptimizedPlan(withCoords, origin, budgets, speed) {
  ...
  for (let di = 0; di < budgets.length; di++) {
    ...
    if (stops.length) {
      travelKm += haversineKm(cur, origin);
    }
...
function routeCoordsForDay(origin, stops) {
  const coords = [origin, ...(stops || []).map((s) => s.coord).filter(Boolean)];
  if (stops && stops.length) coords.push(origin);
  return coords;
}

function computeDayStats(day, origin) {
...
function optimizeDayOrder(day, origin) {
  const remaining = (day.stops || []).slice().filter((s) => !!s.coord);
  const fixed = (day.stops || []).slice().filter((s) => !s.coord); // keep no-coord items at end
  const out = [];
  let cur = origin;
  while (remaining.length) {
    let bestIdx = 0;
...
/* ---------------- Berlin: dropdown pools from ALL POIs ---------------- */
function buildMealPoolsFromAllPois(doc) {
  const pois = doc?.pois || [];
  const lunchPool = [];
  const dinnerPool = [];
  const cocktailPool = [];
  ...
  const byName = (a, b) => (a.poi?.name || "").localeCompare(b.poi?.name || "", "nl", { sensitivity: "base" });
  lunchPool.sort(byName);
  dinnerPool.sort(byName);
  cocktailPool.sort(byName);

  return { lunchPool, dinnerPool, cocktailPool };
}
...
function insertMealSlotsBerlin(plan, origin, budgets, pools) {
  ...
  const wantMealSlot = (di, slot) => {
    ...
    if (slot === "dinner") {
      if (ps.includeDinner === false) return false;
      if (isLast && departure === "morning") return false;
      return true;
    }
    if (slot === "cocktail") {
  ...
  const pickNearestUnused = (pool, anchorCoord) => {
    ...
    for (const x of pool) {
      ...
      const d = x.coord && anchorCoord ? haversineKm(x.coord, anchorCoord) : 1e9;
      if (!best || d < best.d) best = { x, d };
    }
    if (best) {
      usedIds.add(best.x.poi.id);
      return best.x;
    }

    // allow reuse
    const first = pool.find((x) => x?.poi?.id);
    if (first) {
      usedIds.add(first.poi.id);
      return first;
    }
    return null;
  };
  ...
  const pickFromPool = (pool, preferredId, anchorCoord) => {
    ...
    return pickNearestUnused(pool, anchorCoord);
  };
  ...
  for (let di = 0; di < (plan.days || []).length; di++) {
    ...
    if (wantMealSlot(di, "lunch")) {
      ...
      if (pick) {
        const stop = { poi: pick.poi, coord: pick.coord || coordsFromPoi(pick.poi), slot: "lunch", manuallyPlaced: !!lunchPref };
    ...
    if (wantMealSlot(di, "dinner")) {
      ...
      if (pick) {
        const stop = { poi: pick.poi, coord: pick.coord || coordsFromPoi(pick.poi), slot: "dinner", manuallyPlaced: !!dinnerPref };
        dayStops.push(stop);
      }
    }
    ...
    if (wantMealSlot(di, "cocktail")) {
      ...
      if (pick) {
        const stop = { poi: pick.poi, coord: pick.coord || coordsFromPoi(pick.poi), slot: "cocktail", manuallyPlaced: !!cocktailPref };
        dayStops.push(stop);
      }
    }

    day.stops = dayStops;
    computeDayStats(day, origin);
  }
}
...
function ensurePlannerPanel() {
  ...
  style.textContent = `
    ...
    .plan-tools{ margin-top:8px; display:flex; gap:10px; flex-wrap:wrap; align-items:center; }
    ...
    .plan-tools .select{ background: rgba(255,255,255,.03); border:1px solid rgba(255,255,255,.10); color: var(--text); padding:8px 10px; border-radius:12px; }
  `;
...
async function updatePlannerUI() {
  ...
  if (meta) {
    meta.textContent = favList.length ? `${favList.length} favoriet(en) geselecteerd.` : "Selecteer locaties met de ster. Daarna kun je een compacte weekendroute laten voorstellen.";
  }
  if (out && !out.dataset.hasPlan) {
    out.innerHTML = favList.length
      ? `<div class="small">Favorieten:</div><ul>${favList
...
async function buildWeekendPlan() {
  ...
  if (withCoords.length < 1 && !isBerlin) {
  ...
  const origin = getOriginCoord(__DOC) || (withCoords[0]?.coord || { lat: 52.52, lon: 13.405 });
  const speed = avgSpeedKmph(__DOC);
  const budgets = dayBudgets(__PLAN.days, __PLAN.arrival, __PLAN.departure);

  // Berlin: sightseeing = favorites, meals = dropdown from ALL POIs
  let mainStops = withCoords;
  let pools = { lunchPool: [], dinnerPool: [], cocktailPool: [] };

  if (isBerlin && (__PLAN.includeLunch || (__PLAN.includeDinner !== false) || __PLAN.includeCocktails)) {
    pools = buildMealPoolsFromAllPois(__DOC);

    // remove meal-type favorites from sightseeing to avoid duplication
  ...
  if (isBerlin) {
    insertMealSlotsBerlin(plan, origin, budgets, pools);
  } else {
    (plan.days || []).forEach((d) => computeDayStats(d, origin));
  }
  ...
  __CURRENT_PLAN = {
    ...
    htmlProvider: () => buildWeekendPlanHtml(__CURRENT_PLAN.plan, __CURRENT_PLAN.origin, __CURRENT_PLAN.budgets, __CURRENT_PLAN.totalStops, __CURRENT_PLAN.pools),
  };
  savePlanDraft();
...
function buildWeekendPlanHtml(plan, origin, budgets, totalStops, pools) {
  ...
  const mealSelect = (di, slot, label, pool) => {
    ...
    const opts = (pool || [])
      .map((s) => {
        ...
        const loc = s.poi?.location?.locality || "";
        const labelTxt = loc ? `${nm} — ${loc}` : nm;
        return `<option value="${escapeHtml(id)}"${id === chosen ? " selected" : ""}>${escapeHtml(labelTxt)}</option>`;
      })
      .join("");

    return `<label class="small" style="display:flex; flex-direction:column; gap:6px; color:var(--muted);">
      ${label}
      <select class="select" data-action="meal" data-slot="${slot}" data-day="${di}">
  ...
  const section = (label, day, di) => {
    ...
    const coords = routeCoordsForDay(origin, stops.filter((s) => !!s.coord));
    return `
      <div class="plan">
        <h3>${label} <span class="pill">${stops.length} stop(s)</span></h3>

        <div class="plan-tools">
          <label class="small" style="color:var(--muted); display:flex; align-items:center; gap:8px;">
            Start <input class="input time" type="time" data-action="day-start" data-day="${di}" value="${escapeHtml(getDayStart(di))}" />
          </label>

          ${currentDataset === "berlin" ? mealSelect(di, "lunch", "Lunch", pools?.lunchPool) : ""}
          ${currentDataset === "berlin" ? mealSelect(di, "dinner", "Diner", pools?.dinnerPool) : ""}
          ${currentDataset === "berlin" ? mealSelect(di, "cocktail", "Cocktails", pools?.cocktailPool) : ""}

          <button class="btn btn-ghost" data-action="reopt-day" data-day="${di}">Heroptimaliseer deze dag</button>
        </div>

        <div class="small" style="color:var(--muted);">
          Reis: ~${(day.travelKm || 0).toFixed(1)} km • Bezoek: ~${formatMinutes(day.visitMin || 0)} • Budget: ${formatMinutes(day.budgetMin)}
        </div>

        ${stops.length ? `<ul>${stops
          .map(
            (x, si) => `
          <li class="plan-li" data-day="${di}" data-poi="${escapeHtml(x.poi.id)}">
            <div class="li-main">
              <div class="li-title">
                <span class="pill" style="margin-right:8px;">${escapeHtml(hhmmFromMinutes(timeline[si]?.start || 0))}–${escapeHtml(
                  hhmmFromMinutes(timeline[si]?.end || 0)
                )}</span>
                ${escapeHtml(x.poi.name)}
                ${x.poi.type ? `<span class="pill">${escapeHtml(x.poi.type)}</span>` : ""}
                ${x.slot ? `<span class="pill">${escapeHtml(x.slot)}</span>` : ""}
                <span class="pill">${escapeHtml(String(timeline[si]?.dur || ""))} min</span>
              </div>
              <div class="small" style="color:var(--muted);">${escapeHtml(x.poi.location?.locality || "")}</div>
            </div>
            <div class="li-tools">
              <input class="input time" type="time" data-action="time" data-day="${di}" data-poi="${escapeHtml(x.poi.id)}" value="${escapeHtml(
                getManualTime(x.poi.id)
              )}" title="Tijd (optioneel)" />
          ...
          .join("")}</ul>` : `<div class="small">Geen stops gepland (te weinig tijd of alles al ingepland).</div>`}

        ${coords.length >= 2 ? mapsButtons(coords) : ""}
      </div>
    `;
  };

  const allStops = (plan.days || []).flatMap((d) => d.stops || []);
  const withCoords = allStops.filter((x) => !!x?.coord);
  ...
  return `
    ...
    <div class="plan" style="margin-top:10px;">
      ...
      <div class="row" style="margin-top:8px; display:flex; gap:10px; flex-wrap:wrap;">
...
function planDraftKey() {
  ...
  return `tripkit.plan.${currentDataset}.${favIds}.${__PLAN.days}.${__PLAN.arrival}.${__PLAN.departure}.${__PLAN.includeLunch ? "L" : ""}${
    __PLAN.includeDinner !== false ? "D" : ""
  }${__PLAN.includeCocktails ? "C" : ""}`;
}
...
function wirePlanEditor(container) {
  ...
  container.addEventListener("click", (e) => {
    ...
    if (action === "restore") {
      ...
      if (draft) {
        ...
        (__CURRENT_PLAN.plan.days || []).forEach((d) => computeDayStats(d, __CURRENT_PLAN.origin));
        renderCurrentPlan();
      }
      return;
    }
    ...
    if (action === "reopt-day" && dayIdx != null) {
      ...
      computeDayStats(day, __CURRENT_PLAN.origin);
      savePlanDraft();
      renderCurrentPlan();
      return;
    }
    ...
    if (action === "reopt-all") {
      (__CURRENT_PLAN.plan.days || []).forEach((d) => {
        ...
        computeDayStats(d, __CURRENT_PLAN.origin);
      });
      savePlanDraft();
      renderCurrentPlan();
      return;
    ...
    if (action === "up" && idx > 0) {
      const tmp = day.stops[idx - 1];
      day.stops[idx - 1] = day.stops[idx];
      day.stops[idx] = tmp;
      computeDayStats(day, __CURRENT_PLAN.origin);
      savePlanDraft();
      renderCurrentPlan();
  ...
  container.addEventListener("change", async (e) => {
    ...
    if (mealSel) {
      ...
      const val = mealSel.value;

      setMealChoice(currentDataset, di, slot, val === "__none__" ? "" : val);

      // If user picks a specific POI, try to fetch coords so maps links can include it.
      if (val && val !== "__none__") {
        const poi = (__DOC?.pois || []).find((p) => p.id === val);
        if (poi) await ensureCoordsForPois([poi]);
      }
    ...
    if (sel) {
      ...
      computeDayStats(src, __CURRENT_PLAN.origin);
      computeDayStats(dst, __CURRENT_PLAN.origin);
      savePlanDraft();
      renderCurrentPlan();
      return;
    }
...
function createCard(poi, regionById) {
  ...
  getBestWikipediaInfo(poi).then((info) => {
    ...
    if (!info) {
      ...
      if (!alreadyHasImg) PHOTO_STATS.resolved += 1;
      renderPhotoCounter();
      return;
    }
...
function render() {
  ...
  const poisAll = doc.pois || [];
  const pois = poisAll.filter((p) => !shouldHideFromCardList(p));
...
async function loadDataset(ds) {
  ...
  if (els.photosOnly) els.photosOnly.checked = false;
...
async function main() {
  ...
  els.search?.addEventListener("input", (e) => {
    __STATE.search = e.target.value;
    render();
  });
  els.region?.addEventListener("change", (e) => {
    __STATE.region = e.target.value;
    render();
  });
  ...
  els.photosOnly?.addEventListener("change", (e) => {
    __STATE.photosOnly = e.target.checked;
    render();
  });
